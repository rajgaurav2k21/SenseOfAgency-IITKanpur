...
namespace CurvedUI
{
    ...
    public partial class CurvedUIVertexEffect : BaseMeshEffect
    {
        ...
        void CheckTextFontMaterial()
        {
            ...
            if (myText)
            {
                Vector4 savedUV0 = new Vector4(savedTextUV0.x, savedTextUV0.y, 0f, 0f); 
                if (myText.cachedTextGenerator.verts.Count > 0 && myText.cachedTextGenerator.verts[0].uv0 != savedUV0)
                {
        ...
        UIVertex TesselateQuad(float x, float y)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                _uv0 += new Vector2(m_quad[i].uv0.x, m_quad[i].uv0.y) * m_weights[i];
                _uv1 += new Vector2(m_quad[i].uv1.x, m_quad[i].uv1.y) * m_weights[i];
