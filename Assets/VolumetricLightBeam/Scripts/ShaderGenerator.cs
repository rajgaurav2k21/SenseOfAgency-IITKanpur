        ...
        static string GetFolderOutputPath()
{
    ...
    var fullPath = System.IO.Path.GetFullPath(assetPath); // Corrected line
     var fullDir = System.IO.Path.GetDirectoryName(fullPath);
    return fullDir; // full path to shader generator directory
        ...
        class GenShader
        {
            ...
            public Shader Generate(ShaderMode shaderMode)
            {
                ...
                var outputFullPath = System.IO.Path.Combine(outputFolderPath, GetShaderAssetName(shaderMode));
